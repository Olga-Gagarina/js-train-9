...
function findElementGreaterThan(arr, num) {
    ...
    return foundElement !== undefined ? foundElement : "Число, яке задовольняє умову, не знайдено";
...
function findLastEvenNumber(arr) {
    ...
    const lastEvenNumber = arr.findLast((number) => number % 2 === 0);
    
    return lastEvenNumber !== undefined ? lastEvenNumber : 'Парних чисел не знайдено';
...
function flattenSquareAndFilter(arr, threshold) {
    ...
    const squaredArray = flatArray.map((element) => element ** 2);
...
function getLastElementsSumAndPrint(arr, n) {
    ...
    const sum = lastNElements.reduceRight((accumulator, currentValue) => accumulator + currentValue, 0); // Обчислюємо суму останніх n елементів масиву.
...
function expandArrayByN(arr, n) {
    ...
    const expandedArray = arr.flatMap((number) => Array(n).fill(number)); 
